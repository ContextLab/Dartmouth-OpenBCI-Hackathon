...
#sys.path.append("C:\Users\f000hbf\AppData\Local\Continuum\Anaconda2\Lib\site-packages")
sys.path.append('C:\Program Files (x86)\PsychoPy2\lib\site-packages') #added to env vars so works now without this line
...
class OpenBCIBoard(object):
  ...
  def __init__(self, port=None, baud=115200, filter_data=True,
    ...
    if not port:
      port = 'COM4' #know this from previous work, GN edit
      #port = find_port()
      if not port:
    ...
    self.log_packet_count = 0 #GN moved
    ...
    self.read_state = 0
    self.daisy = daisy
    self.last_odd_sample = OpenBCISample(-1, [],[], []) # used for daisy #GN edits
    ...
    while self.streaming:
      ...
      if self.daisy:
        ...
        # even sample: concatenate and send if last sample was the fist part, otherwise drop the packet
          ...
          whole_sample = OpenBCISample(sample.id, sample.channel_data + self.last_odd_sample.channel_data,[], avg_aux_data) #GN edits
  ...
  def _read_serial_binary(self, max_bytes_to_skip=3000):
    ...
    for rep in xrange(max_bytes_to_skip):
      ...
      #---------End Byte---------
        ...
        if (val == END_BYTE):
          systime = (round(time.time() * 1000))
          sample = OpenBCISample(packet_id, channel_data, systime, aux_data)
          self.packets_dropped = 0
          return sample
        else:
...
class OpenBCISample(object):
  ...
  def __init__(self, packet_id, channel_data, systime, aux_data):
    self.id = packet_id;
    newvar = [packet_id,channel_data,aux_data, systime] #GN edited
    self.channel_data = newvar
    #self.channel_data = [channel_data,systime]; #GN edited
...
def handle_sample(sample):
  #need to print out line to datafile
  datafile.write(str(sample.channel_data ))
  datafile.write("\n")
  #datafile.write('/n') #insert new line?
  print(sample.channel_data) #not channels
  
  #systime = int(round(time.time() * 1000)); #GN added
  #print(systime) #GN added 


t = (time.strftime("%d-%m-%Y-%H-%M-%S")) 
datafile = open('.\data\OpenBCIdata-'+t+'.txt', 'w') #GN edit

board = OpenBCIBoard()
board.print_register_settings()
#board.start(handle_sample) #need start_streaming, not start
#make dummy function callback



board.start_streaming(handle_sample)
#board.print_bytes_in()
#stream for certain amount of time 
time.sleep(0.1) #doesnt seem to work? 

board.stop() #end streaming
datafile.close()
